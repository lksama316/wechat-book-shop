...
public class ProductServiceImpl implements IProductService {
    ...
    public ResponseDTO<Boolean> saveProduct(ProductDTO productDTO) {
        ...
        if(DiscountEnum.YES.getCode().equals(productDTO.getIsDiscount())) {
            // 判断折扣价是否低于原价
            if(productDTO.getNewPrice().compareTo(productDTO.getPrice()) > 0) {
                return ResponseDTO.errorByMsg(CodeMsg.PRODUCT_PRICE_ERROR);
            }
        }
    ...
    public ResponseDTO<List<ProductDTO>> getHotProductList() {
        ProductExample productExample = new ProductExample();
        productExample.setOrderByClause("sell_num desc");
    ...
    public ResponseDTO<List<ProductDTO>> getDiscountProductList() {
        ProductExample productExample = new ProductExample();
        productExample.createCriteria().andIsDiscountEqualTo(DiscountEnum.YES.getCode());
        PageHelper.startPage(1, 3);
        List<Product> productList = productMapper.selectByExample(productExample);
        List<ProductDTO> productDTOList = CopyUtil.copyList(productList, ProductDTO.class);
        return ResponseDTO.success(productDTOList);
     ...
     */
    @Override
    public ResponseDTO<List<ProductDTO>> getAllProductList(ProductDTO productDTO) {
        ProductExample productExample = new ProductExample();
        if(CommonUtil.isEmpty(productDTO.getCategoryId())) {
            productExample.createCriteria().andNameLike("%" + productDTO.getName() + "%");
        } else {
            productExample.createCriteria().andNameLike("%" + productDTO.getName() + "%").andCategoryIdEqualTo(productDTO.getCategoryId());
        }
        productExample.setOrderByClause("create_time desc");
        List<Product> productList = productMapper.selectByExample(productExample);
        List<ProductDTO> productDTOList = CopyUtil.copyList(productList, ProductDTO.class);
     ...
     */
    @Override
    public ResponseDTO<ProductDTO> getProductInfo(ProductDTO productDTO) {
        ProductDTO productDTODb = new ProductDTO();
        if(CommonUtil.isEmpty(productDTO.getId())) {
            return ResponseDTO.success(productDTODb);
        }
